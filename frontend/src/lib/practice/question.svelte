		...
		{:else}
			...
			{:then c}
				...
				<div class="flex justify-center">
					<button
						type="button"
						class="w-1/3 text-3xl bg-[#EC4899] my-2 disabled:opacity-60 rounded-lg p-1 transition"
		...
		<div class="flex flex-col w-full h-full gap-4 px-4 py-6">
			{#each question.answers as answer, i (answer.id)}
				<div
					...
					style="background-color: {answer.color ?? '#EC4899'}"
				>
					<button
						onclick={() => {
			...
			{/each}
			<button
				class="bg-[#EC4899] hover:bg-amber-700 text-white font-bold py-2 px-4 rounded-lg mt-2 transition w-full"
				type="button"
